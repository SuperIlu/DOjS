...
static DWORD bitsel (DWORD *input, const int *bitnums, int size)
{
  DWORD ret = 0;
...
void des_set_key (BYTE *key, DESCon *sched)
{
  DWORD  key_msw = GET_32BIT_MSB_FIRST (key);
  DWORD  key_lsw = GET_32BIT_MSB_FIRST (key + 4);
...
static const DWORD SPboxes[8][64] = {
...
static void des_encipher (DWORD *output, DWORD L, DWORD R, DESCon *sched)
{
  DWORD swap, s0246, s1357;
...
static void des_decipher (DWORD *output, DWORD L, DWORD R, DESCon * sched)
{
  DWORD swap, s0246, s1357;
...
static void des_cbc_encrypt (BYTE *dest, const BYTE *src, unsigned int len,
                             DESCon *sched)
{
  DWORD    out[2], iv0, iv1;
  unsigned int i;
...
static void des_cbc_decrypt (BYTE *dest, const BYTE *src, unsigned int len,
                             DESCon *sched)
{
  DWORD    out[2], iv0, iv1, xL, xR;
  unsigned int i;
...
void des_3cbc_encrypt (BYTE *dest, const BYTE *src, unsigned int len, DESCon *scheds)
{
  des_cbc_encrypt (dest, src, len, &scheds[0]);
  des_cbc_decrypt (dest, dest, len, &scheds[1]);
  des_cbc_encrypt (dest, dest, len, &scheds[2]);
}

void des_3cbc_decrypt (BYTE *dest, const BYTE *src, unsigned int len, DESCon *scheds)
{
  des_cbc_decrypt (dest, src, len, &scheds[2]);
  des_cbc_encrypt (dest, dest, len, &scheds[1]);
  des_cbc_decrypt (dest, dest, len, &scheds[0]);
}
