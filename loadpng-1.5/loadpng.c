...
static void read_data(png_structp png_ptr, png_bytep data, png_uint_32 length) {
    ...
    if ((png_uint_32)pack_fread(data, length, f) != length) png_error(png_ptr, "read error (loadpng calling pack_fread)");
}
...
static int check_if_png(PACKFILE *fp) {
    ...
    if (pack_fread(buf, PNG_BYTES_TO_CHECK, fp) != PNG_BYTES_TO_CHECK) return 0;
...
static BITMAP *really_load_png(png_structp png_ptr, png_infop info_ptr, RGB *pal) {
    ...
    if (color_type & PNG_COLOR_MASK_PALETTE) {
        ...
        if (png_get_PLTE(png_ptr, info_ptr, &palette, &num_palette)) {
            ...
            for (; i < 256; i++) pal[i].r = pal[i].g = pal[i].b = 0;
        }
    } else {
    ...
    if ((bpp == 24) || (bpp == 32)) {
        ...
        if (pc[0] == 255) png_set_bgr(png_ptr);
    ...
    for (pass = 0; pass < number_passes; pass++) {
        png_uint_32 y;
        for (y = 0; y < height; y++) png_read_row(png_ptr, bmp->line[y], NULL);
    }

    /* Let Allegro convert the image into the desired colour depth. */
    if (dest_bpp != bpp) bmp = _fixup_loaded_bitmap(bmp, pal, dest_bpp);
...
BITMAP *load_memory_png(AL_CONST void *buffer, int bufsize, RGB *pal) {
    ...
    if (!check_if_png_memory(buffer)) return NULL;
