...
void GrImageFilledBoxAlignC(GrContext *c, int xo, int yo, int x1, int y1, int x2, int y2, GrImage *p) {
    ...
    xo = min(xo, min(x1, x2));
    yo = min(yo, min(y1, y2));
    clip_box(c, x1, y1, x2, y2);
    iwdt = p->pxp_width;
    ihgt = p->pxp_height;
    if ((y2 - y1) <= 0 || (x2 - x1) <= 0 || iwdt <= 0 || ihgt <= 0) return;
    if (c->gc_onscreen)
        bltfun = c->gc_driver->bltr2v;
    else
        bltfun = c->gc_driver->bitblt;
    while (xo > x1) xo -= iwdt;
    while (yo > y1) yo -= ihgt;
    yoff = (y1 - yo) % ihgt;
    yy = y1;
    mouse_block(c, x1, y1, x2, y2);
    x2++;
    y2++;
    do {
        xx = x1;
        ...
        do {
            copyw = min(x2 - xx, iwdt - xoff);
            (*bltfun)(&c->gc_frame, xx + c->gc_xoffset, yy + c->gc_yoffset, &p->pxp_source, xoff, yoff, copyw, copyh, p->pxp_oper);
            xx += iwdt - xoff;
            xoff = 0;
        } while (xx < x2);
        yy += ihgt - yoff;
...
void GrImageDisplayC(GrContext *c, int x, int y, GrImage *p) {
    GRX_ENTER();
    GrImageFilledBoxAlignC(c, x, y, x, y, x + p->pxp_width - 1, y + p->pxp_height - 1, p);
    GRX_LEAVE();
}

void GrImageDisplayExtC(GrContext *c, int x1, int y1, int x2, int y2, GrImage *p) {
    GRX_ENTER();
    GrImageFilledBoxAlignC(c, x1, y1, x1, y1, x2, y2, p);
    GRX_LEAVE();
}
