...
static bool snd_ResetDSP(unsigned int a, unsigned int i, unsigned int d) {
  ...
  if (((inportb(a + 0xE) & 0x80) == 0x80) && (inportb(a + 0xA) == 0xAA)) {
    ...
    if (snd_IRQ == 2 || snd_IRQ == 10 || snd_IRQ == 11) {
      if (snd_IRQ == 2)
        outportb(0xA1, inportb(0xA1) & 253);
      if (snd_IRQ == 10)
        outportb(0xA1, inportb(0xA1) & 251);
      if (snd_IRQ == 11)
        outportb(0xA1, inportb(0xA1) & 247);
...
static void snd_MixVoice(struct WaveData *Voice) {
  ...
  */
  ...
  if (OffSet > beforeOffSet)
    dosmemput(snd_DMABuffer + beforeOffSet, OffSet - beforeOffSet,
              snd_DOSBufOfs + beforeOffSet);
  else {
    dosmemput(snd_DMABuffer + beforeOffSet, 32768 - beforeOffSet,
              snd_DOSBufOfs + beforeOffSet);
...
static bool snd_LoadVoice(struct WaveData *Voice, const char *FileName) {
  ...
  if (WAVFile == NULL) {
    ...
    DEBUGF("Unable to open wave file %s\n", FileName);
    return false;
  }
  ...
  if (buffer[0] != 'R' || buffer[1] != 'I' || buffer[2] != 'F' ||
      buffer[3] != 'F' || buffer[8] != 'W' || buffer[9] != 'A' ||
      buffer[10] != 'V' || buffer[11] != 'E' || buffer[12] != 'f' ||
      buffer[13] != 'm' || buffer[14] != 't' || buffer[15] != ' ') {
    DEBUGF("No wave file %s\n", FileName);
    fclose(WAVFile);
    return false;
  }
  ...
  DEBUGF("WAV %s is format=%d, channels=%d, rate=%ld and %d bits\n", FileName,
         Voice->format, Voice->channels, Voice->rate, Voice->num_bits);

  if (Voice->format != 1 || Voice->channels != 1 || Voice->num_bits != 8) {
    DEBUGF("WAV %s must be PCM, 8bit, mono!\n", FileName);
    fclose(WAVFile);
    return false;
  }
  if (Voice->rate != 11025) {
    DEBUGF("WARNING: WAV %s samplerate != 11025!\n", FileName);
  }
  ...
  Voice->Sample =
      (unsigned char *)malloc(Voice->SoundLength + 2); // Assign memory
  if (!Voice->Sample) {
    //..display error message
    DEBUG("Can't malloc() sample\n");
    fclose(WAVFile);
    return false;
  }
...
bool init_sbsound(js_State *J) {
  ...
  if (detect_sb(&blaster)) {
    ...
    js_getproperty(J, -1, "prototype");
    { PROTDEF(J, SBSound_play, TAG_SBSOUND, "Play", 2); }
