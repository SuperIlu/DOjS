...
function MakeQR(bmw, bmh, text) {
	...
	qrcode.addData(text);
	qrcode.make();
	Println("2 " + qrcode.getModuleCount());

	var bm = new Bitmap(bmw, bmh);
	SetRenderBitmap(bm);

	// compute tileW/tileH based on options.width/options.height
	var tileW = bm.width / qrcode.getModuleCount();
	var tileH = bm.height / qrcode.getModuleCount();

	// draw in the canvas
	for (var row = 0; row < qrcode.getModuleCount(); row++) {
		for (var col = 0; col < qrcode.getModuleCount(); col++) {
			var c = qrcode.isDark(row, col) ? EGA.BLACK : EGA.WHITE;
			var x = Math.round(col * tileW);
			var y = Math.round(row * tileH);
			var w = (Math.ceil((col + 1) * tileW) - Math.floor(col * tileW));
			var h = (Math.ceil((row + 1) * tileH) - Math.floor(row * tileH));
			FilledBox(x, y, x + w, y + h, c);
		}
	}
