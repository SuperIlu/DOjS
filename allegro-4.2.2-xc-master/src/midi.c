...
MIDI *load_midi_pf(PACKFILE *fp) {
    ...
    for (c = 0; c < num_tracks; c++) { /* read each track */
        ...
        if (pack_fread(midi->track[c].data, data, fp) != data) goto err;
    }
...
static INLINE int global_volume_fix(int vol) {
    if (_midi_volume >= 0) return (vol * _midi_volume) / 256;
 ...
 */
static INLINE int sort_out_volume(int c, int vol) { return global_volume_fix((vol * midi_channel[c].volume) / 128); }
...
static void midi_note_off(int channel, int note) {
    ...
    if (midi_driver->raw_midi) {
        if (channel != 9) note += patch_table[midi_channel[channel].patch].pitch;
    ...
    if (!done) {
        for (c = 0; c < MIDI_VOICES; c++) {
            if ((midi_waiting[c].channel == channel) && (midi_waiting[c].note == note)) {
...
int _midi_allocate_voice(int min, int max) {
    ...
    if (max < 0) max = midi_driver->voices - 1;

    /* which layer can we use? */
    for (layer = 0; layer < MIDI_LAYERS; layer++)
        if (midi_channel[midi_alloc_channel].note[midi_alloc_note][layer] < 0) break;

    if (layer >= MIDI_LAYERS) return -1;

    /* find a free voice */
    for (c = min; c <= max; c++) {
        if ((midi_voice[c].note < 0) && (midi_voice[c].time < best_time) && ((c < midi_driver->xmin) || (c > midi_driver->xmax))) {
            voice = c;
    ...
    if (voice < 0) {
        ...
        best_time = LONG_MAX;
        for (c = min; c <= max; c++) {
            if ((midi_voice[c].time < best_time) && ((c < midi_driver->xmin) || (c > midi_driver->xmax))) {
                voice = c;
                best_time = midi_voice[c].time;
            }
        }
...
static void midi_note_on(int channel, int note, int vol, int polite) {
    ...
    if (channel != 9) {
        ...
        for (c = 0; c < midi_driver->voices; c++)
            if ((midi_voice[c].note < 0) && ((c < midi_driver->xmin) || (c > midi_driver->xmax))) break;
    ...
    midi_driver->key_on(inst, corrected_note, bend, sort_out_volume(channel, vol), midi_channel[channel].pan);
}
...
static void all_notes_off(int channel) {
    if (midi_driver->raw_midi) {
        ...
        for (note = 0; note < 128; note++)
            for (layer = 0; layer < MIDI_LAYERS; layer++)
                if (midi_channel[channel].note[note][layer] >= 0) midi_note_off(channel, note);
    }
}
...
static void process_meta_event(AL_CONST unsigned char **pos, long *timer) {
    ...
    if (midi_meta_callback) midi_meta_callback(metatype, *pos, length);
...
static void process_midi_event(AL_CONST unsigned char **pos, unsigned char *running_status, long *timer) {
    ...
    if (event & 0x80) { /* regular message */
        ...
        if ((event != 0xF0) && (event != 0xF7) && (event != 0xFF)) *running_status = event;
        byte1 = (*pos)[0];
        byte2 = (*pos)[1];
    ...
    /* program callback? */
    if ((midi_msg_callback) && (event != 0xF0) && (event != 0xF7) && (event != 0xFF)) midi_msg_callback(event, byte1, byte2);
    ...
    switch (event >> 4) {
        ...
        case 0x0C: /* program change */
            ...
            if (midi_driver->raw_midi) raw_program_change(channel, byte1);
            (*pos) += 1;
            break;
        ...
        case 0x0F: /* special event */
            switch (event) {
                ...
                case 0xF7:
                    l = parse_var_len(pos);
                    if (midi_sysex_callback) midi_sysex_callback(*pos, l);
                    (*pos) += l;
                    break;
...
do_it_all_again:
    ...
    for (c = 0; c < MIDI_VOICES; c++) midi_waiting[c].note = -1;
    ...
    for (c = 0; c < MIDI_TRACKS; c++) {
        if (midi_track[c].pos) {
            ...
            while (midi_track[c].timer <= 0) {
                process_midi_event((AL_CONST unsigned char **)&midi_track[c].pos, &midi_track[c].running_status, &midi_track[c].timer);
    ...
    for (c = 0; c < MIDI_TRACKS; c++) {
        if (midi_track[c].pos) {
            active = 1;
            if (midi_track[c].timer < midi_timer_speed) midi_timer_speed = midi_track[c].timer;
        }
    }
    ...
    /* reprogram the timer */
    if (midi_timer_speed < BPS_TO_TIMER(MIDI_TIMER_FREQUENCY)) midi_timer_speed = BPS_TO_TIMER(MIDI_TIMER_FREQUENCY);

    if (!midi_seeking) install_int_ex(midi_player, midi_timer_speed);
    ...
    for (c = 0; c < MIDI_VOICES; c++)
        if (midi_waiting[c].note >= 0) midi_note_on(midi_waiting[c].channel, midi_waiting[c].note, midi_waiting[c].volume, 0);
...
static int midi_init(void) {
    ...
    for (c = 0; c < 16; c++) {
        ...
        for (c2 = 0; c2 < 128; c2++)
            for (c3 = 0; c3 < MIDI_LAYERS; c3++) midi_channel[c].note[c2][c3] = -1;
    }
 ...
 */
static void midi_exit(void) { stop_midi(); }
...
static void prepare_to_play(MIDI *midi) {
    ...
    for (c = 0; c < 16; c++) reset_controllers(c);
    ...
    for (c = 0; c < 16; c++) {
        ...
        if (midi_driver->raw_midi) raw_program_change(c, 0);
    }
...
int play_midi(MIDI *midi, int loop) {
    ...
    if (midi) {
        if (!midi_loaded_patches)
            if (load_patches(midi) != 0) return -1;
 ...
 */
void stop_midi(void) { play_midi(NULL, FALSE); }
...
int midi_seek(int target) {
    ...
    if (target <= midi_pos) prepare_to_play(midifile);
    ...
    if (midi_pos >= 0) {
        ...
        if (midi_driver->raw_midi) {
            for (c = 0; c < 16; c++) {
                ...
                if ((midi_channel[c].patch != old_patch[c]) || (midi_channel[c].volume != old_volume[c])) raw_program_change(c, midi_channel[c].patch);
...
int get_midi_length(MIDI *midi) {
    play_midi(midi, 0);
    while (midi_pos < 0)
        ; /* Without this, midi_seek won't work. */
    midi_seek(INT_MAX);
    return midi_time;
}
...
void midi_out(unsigned char *data, int length) {
    ...
    while (pos < data + length) process_midi_event((AL_CONST unsigned char **)&pos, &running_status, &timer);
...
int load_midi_patches(void) {
    ...
    for (c = 0; c < 128; c++) patches[c] = drums[c] = TRUE;
...
static struct _AL_LINKER_MIDI midi_linker = {midi_init, midi_exit};

void _midi_constructor(void) { _al_linker_midi = &midi_linker; }
